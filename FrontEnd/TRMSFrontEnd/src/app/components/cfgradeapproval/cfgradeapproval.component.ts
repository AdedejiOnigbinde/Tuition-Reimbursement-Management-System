import { Component, OnInit } from '@angular/core';
import { ActivatedRoute, Router } from '@angular/router';
import { Form } from 'src/app/models/Form';
import { Grade } from 'src/app/models/Grade';
import { FormService } from 'src/app/services/form.service';

@Component({
  selector: 'app-cfgradeapproval',
  templateUrl: './cfgradeapproval.component.html',
  styleUrls: ['./cfgradeapproval.component.css']
})
export class CfgradeapprovalComponent implements OnInit {
  formId: string = '';
  sApproval: boolean = false;
  grade: Grade = new Grade();
  formValue: Form = new Form();
  reject: boolean = false;
  constructor(private formServices: FormService, private param: ActivatedRoute, private route: Router) { }

  ngOnInit(): void {
    const id = this.param.snapshot.params['id'];
    this.getUserForms(id);
    this.formId = id;
  }

  getUserForms(id: string) {
    this.formServices.getForm(id).subscribe(
      (response) => {
        this.formValue = response;
      }
    );
  }

  getGrade() {
    this.formServices.getGrade(this.formId).subscribe(
      (response) => {
        this.grade = response;
      }
    );
  }

  setApproval() {
    this.sApproval = true;
    this.formServices.setCFApproval(this.formId, this.sApproval).subscribe(
      (response) => {
        this.route.navigate(['cfapproval'])
      }
    );
  }

  rejection() {
    this.reject = true;
    this.formServices.rejectForm(this.formId, this.reject).subscribe(
      (response) => {
        this.route.navigate(['cfapproval'])
      }
    )
  }

}
